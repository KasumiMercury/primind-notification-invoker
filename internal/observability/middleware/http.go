...
import (
	...
	"github.com/KasumiMercury/primind-notification-invoker/internal/observability/tracing"
...
func HTTP(next http.Handler, cfg HTTPConfig) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if cfg.HTTPMetrics != nil {
			metricsPath := r.Pattern
			if metricsPath == "" {
				metricsPath = r.URL.Path
			}
			cfg.HTTPMetrics.Record(ctx, r.Method, metricsPath, wrapped.status, duration)
		}
